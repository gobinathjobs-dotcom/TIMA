    ...
    return (
        ...
        >
            ...
            <div className={`max-w-[1920px] mx-auto px-8 md:px-16 lg:px-24 xl:px-32 flex items-center transition-all duration-700 ${isScrolled ? 'h-12 md:h-14' : 'h-20 md:h-28'}`}>
                ...
                {/* Right Column: Rows */}
                <div className="hidden md:flex flex-col justify-center h-full ml-auto">
                    {/* Top Row: Utilities - hidden when scrolled */}
                    {!isScrolled && (
                        <div className="flex justify-between items-center pt-3 pb-6 border-b border-white text-[13px] text-white/95">
                    ...
                    <div className={`flex justify-between items-center ${isScrolled ? 'py-0' : 'pt-4'}`}>
                        <div className="flex items-center gap-6 lg:gap-8">
                            {navLinks.map((link) => (
                                <button
                                    ...
                                    className={`flex items-center gap-2 text-[16px] font-medium tracking-wide transition-colors ${activeDropdown === link.label
                                        ? 'text-white'
                        ...
                        <div className={`flex items-center gap-5 ml-8 lg:ml-16 text-white`}>
                ...
                {/* Mobile menu toggle */}
                <button
                    className={`md:hidden p-2 ml-auto self-center transition-colors text-white`}
            ...
            {activeDropdown && (
                <div
                    className="hidden md:block absolute left-0 right-0 bg-white shadow-xl border-t-2 border-tima-gold"
            ...
            {/* Mobile menu */}
            {mobileOpen && (
                <div className="md:hidden bg-[#1a2234] border-t border-white/10 px-6 py-4 max-h-[70vh] overflow-y-auto">
                    {navLinks.map((link) => (
                        <div key={link.label}>
                            <button
                                onClick={() => toggleMobileExpanded(link.label)}
                                className={`flex items-center justify-between w-full py-3 transition-colors border-b border-white/5 text-sm font-bold ${mobileExpanded === link.label ? 'text-tima-gold' : 'text-white/90 hover:text-tima-gold'
                                    }`}
                            >
                                {link.label}
                                <ChevronDown
